...
/* =========================== Controller ============================= */
NominalMPC::NominalMPC(double _dt, int _iterations_between_mpc):
 ...
 //Integral-esque pitch and roll compensation
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    //if((swingTimeRemaining[i]*1000)%5 == 0)  
      footSwingTrajectories[i].setHeight(.12);
...
// + interleave_y[i] * interleave_gain * v_abs;
 ...
 phase = gait->_phase;
  ...
  for(int foot = 0; foot < 4; foot++)
  {
   ...
   //std::cout << " height " << seResult.position(2) << std:: endl;
	...
	//	        vDesLeg<< 0, 0, 0;
        ...
        data._legController->commands[foot].kpJoint.diagonal() << 80, 80, 80;
